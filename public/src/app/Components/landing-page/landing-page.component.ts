...
export class LandingPageComponent implements OnInit, OnDestroy {
  ...
  private initSocketConnection(): void {
    ...
    this.socketNewRoomObs = this.socketService.onRoomCreated().subscribe(
      (res) => {
        this.spinner.hide();
    ...
    this.socketJoinRoomObs = this.socketService.onRoomJoined().subscribe(
      (res) => {
        this.spinner.hide();
        if (res.retCode === ApiReturnEnum.Successful) {
          ...
          if (joinGrpRes.isValidGroup) {
            ...
            this.toastrService.error(
              "Invalid room Id. <br> Please enter a valid room id",
              "",
              { enableHtml: true }
            );
          }
